...
export function ModulePage({ section, siteId, site, moduleData }: { section: string; siteId?: string; site?: SiteSettings | null; moduleData: ModuleData }) {
  ...
  const summary = buildModuleSummary(section, moduleData);
  return <div className="mx-auto max-w-[1500px] animate-rise">
    <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end"><div><h1 className="text-3xl font-semibold tracking-[-.03em]">{copy.title}</h1><p className="mt-2 max-w-2xl text-sm text-slate-500">{copy.description}</p></div><div className="flex gap-2"><SiteActionButton siteId={siteId} action="export" variant="outline">导出 URL</SiteActionButton><SiteActionButton siteId={siteId} action={isSettings ? "settings" : isGeo ? "geo" : isBotLogs ? "logs" : "sync"} target={isSettings ? "google-search-console" : undefined}>{isSettings ? "添加集成" : isGeo ? "运行 GEO 测试" : isBotLogs ? "扫描日志" : "同步数据"}</SiteActionButton></div></div>
    <div className="mt-7 grid gap-4 md:grid-cols-[.72fr_1.28fr]">
      ...
      <Panel className="p-6"><div className="flex items-center justify-between"><div><h2 className="font-semibold">数据就绪状态</h2><p className="mt-1 text-xs text-slate-500">当前模块的集成和同步状态</p></div><Badge tone={summary.ready ? "good" : "warn"}>{summary.ready ? "已就绪" : "暂无记录"}</Badge></div><div className="mt-6 grid gap-3 sm:grid-cols-3">{summary.items.map((item) => <div key={item.label} className="rounded-xl bg-[#f7f9fa] p-4"><p className="text-xs text-slate-500">{item.label}</p><p className="mt-2 text-sm font-semibold text-slate-500">{item.value}</p></div>)}</div><p className="mt-5 text-xs leading-5 text-slate-500">这里只展示真实服务商结果和已保存的站点配置。</p></Panel>
    </div>
    {isSettings ? <SettingsForm site={site ?? null} /> : <ModuleDataView section={section} data={moduleData} emptyTitle={copy.emptyTitle} emptyDescription={copy.emptyDescription} />}
  </div>;
}

function ModuleDataView({ section, data, emptyTitle, emptyDescription }: { section: string; data: ModuleData; emptyTitle: string; emptyDescription: string }) {
  ...
  if (section === "reports") return <ReportsTable data={data} emptyTitle={emptyTitle} emptyDescription={emptyDescription} />;
...
function ReportsTable({ data, emptyTitle, emptyDescription }: { data: ModuleData; emptyTitle: string; emptyDescription: string }) {
...
function SectionTitle({ title, description }: { title: string; description: string }) {
  return <div><h3 className="text-lg font-semibold">{title}</h3><p className="mt-1 text-sm text-slate-500">{description}</p></div>;
}
