...
export function ModulePage({ section, siteId, site, moduleData }: { section: string; siteId?: string; site?: SiteSettings | null; moduleData: ModuleData }) {
  ...
  const primaryAction = isSettings ? "settings" : isGeo ? "geo" : isBotLogs ? "logs" : isReports ? "report" : isTechnicalSeo ? "technical" : "sync";
  const primaryLabel = isSettings ? "\u6dfb\u52a0\u96c6\u6210" : isGeo ? "\u8fd0\u884c GEO \u6d4b\u8bd5" : isBotLogs ? "\u626b\u63cf\u65e5\u5fd7" : isReports ? "\u751f\u6210\u62a5\u544a" : isTechnicalSeo ? "\u8fd0\u884c\u6293\u53d6" : "\u540c\u6b65\u6570\u636e";
  ...
  return <div className="mx-auto max-w-[1500px] animate-rise">
    <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end"><div><h1 className="text-3xl font-semibold tracking-[-.03em]"><GlossaryLabel>{copy.title}</GlossaryLabel></h1><p className="mt-2 max-w-2xl text-sm text-slate-500">{copy.description}</p></div><div className="flex gap-2"><SiteActionButton siteId={siteId} action="export" variant="outline">导出 URL</SiteActionButton><SiteActionButton siteId={siteId} action={primaryAction} target={isSettings ? "google-search-console" : undefined}>{primaryLabel}</SiteActionButton></div></div>
